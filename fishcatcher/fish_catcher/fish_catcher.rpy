...
init:
    image bg background = "backforgame.webp"
    ...
    python:
        ...
        class Fish():
            ...
            def createNew( self ):
                self.position[0] = random.randrange( 0, 13 ) * 100
            ...
            def render( self, renderer, shownTimebase, animationTimebase ):
                if ( self.active ):
                    r = renpy.render( self.image, 1280, 720, shownTimebase, animationTimebase )
            ...
            def update( self, deltaTime ):
                if ( self.grabCounter > 0 ):
                    ...
                    if ( self.position[0] < 0 ):
                        self.position[0] = 0
                    elif ( self.position[0] > 1280 - self.dimensions[0] ):
                        self.position[0] = 1280 - self.dimensions[0]
            ...
            def render( self, width, height, shownTimebase, animationTimebase ):
                ...
                if ( self.gameover == False ):
                    ...
                    for debug in self.debug:
                        ...
                        textRender = renpy.render( txt, 1280, 720, shownTimebase, animationTimebase )
                    ...
                    renderer.blit( renpy.render( txt, 1280, 720, shownTimebase, animationTimebase ), ( 500, 300 ) )


                txtScore = Text( _( "Время: " + str( self.countdown ) ), size=20 )
                renderer.blit( renpy.render( txtScore, 1280, 720, shownTimebase, animationTimebase ), ( 1100, 0 ) )

                txtScore = Text( _( "Счет: " + str( self.player.score ) ), size=20 )
                renderer.blit( renpy.render( txtScore, 1280, 720, shownTimebase, animationTimebase ), ( 1100, 20 ) )
